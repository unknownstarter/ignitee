...
export async function POST(request: NextRequest) {
  try {
    const { prd, action, conversationHistory = [] } = await request.json();
    ...
    const response = await fetch(`${orchestratorUrl}/api/generate-strategy`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ prd, action, conversationHistory }),
    });
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: '전략 생성 중 오류가 발생했습니다.' }, { status: 500 });
    }
